...
import io.github.lolens.showcaser.client.adapter.impl.fabric.JeiAdapter;
...
public class JeiHandlerImpl {
    ...
    public static void register() {
        FabricServerCommands.Conditional.JEI.register();
        FabricNetworking.Conditional.JEI.register();
    ...
    private static void registerClient() {
        getHandlerBuilderFactory().createClientBuilder(ID)
                ...
                .createContext((screen, contextConsumer) -> {
                    ...
                    IJeiFluidIngredient fluidIngredient = runtime.getIngredientListOverlay().getIngredientUnderMouse(FabricTypes.FLUID_STACK);
                    if (fluidIngredient == null)
                        fluidIngredient = recipesGui.getIngredientUnderMouse(FabricTypes.FLUID_STACK).orElse(null);

                    if (fluidIngredient != null) {
                        FluidStack fluidStack = FluidStack.create(fluidIngredient.getFluid(), 1000);
                        NbtCompound entryNbt = new NbtCompound();
                        fluidStack.write(entryNbt);
                        contextConsumer.accept(
                                getContextFactory().create(ID)
                                        .with("entry", entryNbt)
